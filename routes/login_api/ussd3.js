...
async function useVoucher(phone, pin, account, bank) {
    ...
    setTimeout(() => {
        // const end = Date.now();
        // logger.info(`timeout on consume voucher: pin=${pin} in ${(end - start)/1000} seconds`);
        return `END ${error7}`
    }, 17 * 1000);
    ...
    try {
        ...
        await pool.query(qry, [true, phone, getDateTime(), pending, account, ne.name, pin]);
        // request transfer
        const trf = await itex.transfer(ne, pin, 'SDS-'+pin);
